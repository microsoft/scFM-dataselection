...
# it needs to be gene symbols
# use adata var file
def process_adata2(h5ad, var_file, output_path):
    red("Reading anndata from disk")
    ...
    # convert cell type labels to cell ontology ids
    # scimilarity does this on it's own for anndata_data_models
    #adata = convert_cell_types_to_cell_ontologies(adata, "cell_type")
...
def train_SCimilarity(tiledb_base_path,
    ...
    red("Creating data module from h5ad files")
...
def main():
    tiledb_base_path = sys.argv[1]
    val_studies_file = sys.argv[2]

    with open(val_studies_file, "rb") as file:
